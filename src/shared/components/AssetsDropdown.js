...
export default function AssetsDropdown() {
  ...
  return (
    <div className="relative" onMouseEnter={openDropdownPreview} onMouseLeave={scheduleCloseDropdown} ref={dropdownRef}>
      {/* Assets Button */}
      <button
        className="flex h-10 items-center gap-1 rounded-full border border-[#bbcac1]/30 bg-surface-container-low px-3 text-[#3c4a43] transition-colors hover:bg-surface-container-high"
      ...
      >
        ...
        <span className="hidden text-sm font-semibold md:inline">Tài sản</span>
      </button>

      {/* Dropdown Popup */}
      {isOpen && (
        <div
          className="absolute right-0 top-full w-[380px] rounded-2xl border border-[#bbcac1]/20 bg-white shadow-[0_12px_32px_rgba(0,0,0,0.08)] backdrop-blur-xl"
